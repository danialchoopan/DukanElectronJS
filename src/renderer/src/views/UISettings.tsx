...
export default function UISettings() {
  const { theme, setTheme, language, setLanguage: setLang, navTheme, setNavTheme, fontSize, setFontSize, fontSizeCustom, setFontSizeCustom, highContrast, setHighContrast } = useSettingsStore()
  const ui = t()
    ...
    <div className="space-y-6 max-w-2xl">
      <div className="rounded-2xl p-5 border" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button onClick={() => { setLang('fa'); setLanguage('fa') }}
            className="py-3 rounded-xl text-lg font-bold transition-all"
            style={{
              background: language === 'fa' ? 'linear-gradient(135deg, #3b82f6, #2563eb)' : 'var(--bg-tertiary)',
              color: language === 'fa' ? '#ffffff' : 'var(--text-secondary)',
            }}>فارسی</button>
          <button onClick={() => { setLang('en'); setLanguage('en') }}
            className="py-3 rounded-xl text-lg font-bold transition-all"
            style={{
              background: language === 'en' ? 'linear-gradient(135deg, #3b82f6, #2563eb)' : 'var(--bg-tertiary)',
      ...
      <div className="rounded-2xl p-5 border" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
        ...
        <div className="grid grid-cols-3 gap-2">
          <button onClick={() => setTheme('dark')} className="py-3 rounded-xl text-sm font-bold transition-all"
            style={{
              background: theme === 'dark' ? 'linear-gradient(135deg, #1e293b, #0f172a)' : 'var(--bg-tertiary)',
              color: theme === 'dark' ? '#ffffff' : 'var(--text-secondary)',
              border: theme === 'dark' ? '2px solid #3b82f6' : '2px solid transparent',
            }}>
            {ui.admin.darkMode}
          </button>
          <button onClick={() => setTheme('light')} className="py-3 rounded-xl text-sm font-bold transition-all"
            style={{
              background: theme === 'light' ? 'linear-gradient(135deg, #f1f5f9, #e2e8f0)' : 'var(--bg-tertiary)',
              color: theme === 'light' ? '#0f172a' : 'var(--text-secondary)',
              border: theme === 'light' ? '2px solid #3b82f6' : '2px solid transparent',
            }}>
            {ui.admin.lightMode}
          </button>
          <button onClick={() => setHighContrast(!highContrast)} className="py-3 rounded-xl text-sm font-bold transition-all"
            style={{
              background: highContrast ? 'linear-gradient(135deg, #000000, #1a1a1a)' : 'var(--bg-tertiary)',
              color: highContrast ? '#ffffff' : 'var(--text-secondary)',
              border: highContrast ? '2px solid #f59e0b' : '2px solid transparent',
            }}>
            کنتراست بالا
          </button>
        </div>
          ...
          {themes.map((th) => (
            <button key={th.key} onClick={() => setNavTheme(th.key)}
              ...
              style={{
                ...
                border: navTheme === th.key ? '3px solid #ffffff' : '3px solid transparent',
                transform: navTheme === th.key ? 'scale(1.05)' : 'scale(1)',
                boxShadow: navTheme === th.key ? '0 4px 12px rgba(0,0,0,0.3)' : 'none',
              }}>
              {th.name}
            </button>
          ))}
      ...
      <div className="rounded-2xl p-5 border" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
        ...
        <div className="grid grid-cols-5 gap-2 mb-4">
          {fontSizes.map((fs) => (
            <button key={fs.key} onClick={() => setFontSize(fs.key)}
              ...
              style={{
                background: fontSize === fs.key ? 'linear-gradient(135deg, #3b82f6, #2563eb)' : 'var(--bg-tertiary)',
                color: fontSize === fs.key ? '#ffffff' : 'var(--text-secondary)',
                textShadow: fontSize === fs.key ? '0 1px 3px rgba(0,0,0,0.3)' : 'none',
                fontSize: `${fs.scale * 14}px`,
              }}>
              {fs.label}
            </button>
          ))}
        ...
        <div className="mt-4">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-xs font-bold px-2 py-0.5 rounded" style={{ backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-primary)' }}>
              {Math.round(fontSizeCustom * 100)}%
            </span>
          </div>
          <input
            type="range"
            ...
            onChange={(e) => setFontSizeCustom(parseFloat(e.target.value))}
        ...
        <p className="text-xs mt-2" style={{ color: 'var(--text-muted)' }}>پیش‌نمایش: متن تستی با اندازه انتخاب شده</p>
      </div>
    </div>
  )
}
