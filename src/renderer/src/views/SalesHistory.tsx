...
export default function SalesHistory() {
  ...
  const handleReturn = (sale: Sale) => {
    setReturnedIds((prev) => new Set(prev).add(sale.id))
  ...
  const btnColor = isDark ? textSecondary : '#ffffff'
  ...
  const returnCount = filteredSales.filter(s => returnedIds.has(s.id)).length
    ...
    <div className="h-full p-4 overflow-auto">
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <div className="flex-1 min-w-[200px]">
          <label className="text-xs font-medium block mb-1" style={{ color: textSecondary }}>جستجو</label>
          <input ref={searchRef} type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
            className="input-field" placeholder="نام مشتری، فاکتور، مبلغ... (F2)" />
        </div>
        ...
        <div className="flex items-end gap-2">
          ...
          <button onClick={() => { setStartDate(''); setEndDate(''); setSearchQuery(''); setFilterMethod('all'); setFilterStatus('all'); }}
            className="text-sm px-4 py-2 rounded-xl font-bold" style={{ backgroundColor: isDark ? '#334155' : '#f1f5f9', color: textSecondary }}>
            {fa.dashboard.all}
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-3">
        {['all', 'cash', 'card', 'ledger'].map((m) => (
          <button key={m} onClick={() => setFilterMethod(m)}
            ...
            style={{
              backgroundColor: filterMethod === m ? '#3b82f6' : (isDark ? '#334155' : '#f1f5f9'),
              color: filterMethod === m ? '#ffffff' : btnColor,
            }}>
        ...
        {['all', 'normal', 'returned'].map((s) => (
          <button key={s} onClick={() => setFilterStatus(s)}
            className="px-4 py-1.5 rounded-full text-xs font-bold transition-all"
            style={{
              backgroundColor: filterStatus === s ? (s === 'returned' ? '#f59e0b' : '#3b82f6') : (isDark ? '#334155' : '#f1f5f9'),
              color: filterStatus === s ? '#ffffff' : btnColor,
            }}>
            {s === 'all' ? 'همه' : s === 'normal' ? 'عادی' : 'بازگشتی'}
            {s === 'returned' && returnCount > 0 ? ` (${returnCount})` : ''}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        ...
        {returnCount > 0 && (
          <div className="rounded-xl p-3 text-center" style={{ backgroundColor: isDark ? '#451a03' : '#fef3c7', border: `1px solid ${cardBorder}` }}>
            <div className="text-xs" style={{ color: textSecondary }}>بازگشتی</div>
            <div className="text-xl font-bold" style={{ color: '#f59e0b' }}>{returnCount}</div>
          </div>
        )}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filteredSales.map((s, idx) => {
              ...
              return (
                <tr key={s.id} className="transition-all cursor-pointer" style={{
                  borderBottom: `1px solid ${cardBorder}`,
                  backgroundColor: isReturned ? (isDark ? 'rgba(245,158,11,0.08)' : 'rgba(245,158,11,0.06)') : 'transparent',
                }}
                  ...
                  <td className="px-4 py-2.5 font-bold text-right" style={{ color: isReturned ? '#f59e0b' : textPrimary, textDecoration: isReturned ? 'line-through' : 'none' }}>
                    {s.total_amount.toLocaleString('fa-IR')}
                  </td>
                  <td className="px-4 py-2.5 text-center">
                    {isReturned ? (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold" style={{ backgroundColor: '#fef3c7', color: '#92400e' }}>بازگشتی</span>
                    ) : (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold" style={{ backgroundColor: '#dcfce7', color: '#166534' }}>عادی</span>
                    )}
                  </td>
            ...
            {filteredSales.length === 0 && (
              <tr><td colSpan={9} className="text-center py-12" style={{ color: textSecondary }}>{fa.dashboard.noData}</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {selectedSale && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center" onClick={() => setSelectedSale(null)}>
          <div className="rounded-2xl p-6 max-w-lg w-full mx-4 border-2" style={{
            backgroundColor: cardBg,
            borderColor: returnedIds.has(selectedSale.id) ? '#f59e0b' : '#3b82f6',
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                ...
                {returnedIds.has(selectedSale.id) && (
                  <span className="inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] font-bold" style={{ backgroundColor: '#fef3c7', color: '#92400e' }}>بازگشت شده</span>
                )}
              </div>
            ...
            <div className="flex gap-2">
              <button onClick={() => { window.print() }} className="btn-primary flex-1 py-2">چاپ مجدد فاکتور</button>
              {!returnedIds.has(selectedSale.id) && (
                <button onClick={() => handleReturn(selectedSale)} className="btn-danger flex-1 py-2">بازگشت کالا</button>
              )}
              {returnedIds.has(selectedSale.id) && (
                <div className="flex-1 py-2 text-center text-sm font-bold rounded-lg" style={{ backgroundColor: '#fef3c7', color: '#92400e' }}>
                  این فاکتور بازگشت شده
                </div>
              )}
            </div>
          </div>
