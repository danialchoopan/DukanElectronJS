...
function formatProduct(row: Record<string, any>): Product {
  return {
    ...
    unit: (row.unit as 'number' | 'weight') || 'number',
    ...
    isLoose: row.isLoose == 1 || row.isLoose === true,
    isActive: row.isActive == 1 || row.isActive === true,
    isSellable: row.isSellable != 0,
    hasExpiry: row.has_expiry == 1,
    expiryDate: String(row.expiry_date ?? ''),
    expiryAlertDays: Number(row.expiry_alert_days) || 30,
    lastAlerted: row.last_alerted == 1,
