...
export function seedDatabase(): void {
  ...
  if (existingSaleCount.c === 0) {
    const insertSale = db.prepare('INSERT INTO sales (invoiceNumber, userId, customerId, subtotal, total_amount, totalNetProfit, paymentMethod, customerPaid, changeAmount, createdAt, saleDate, affectsInventory, saleType) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
    ...
    const salesTx = db.transaction(() => {
      for (let day = 0; day < 45; day++) {
        ...
        for (let s = 0; s < numSales; s++) {
          ...
          const r = insertSale.run(invoiceNum, userId, customerId, subtotal, subtotal, netProfit, paymentMethod, paymentMethod === 'ledger' ? 0 : subtotal, 0, saleDate, saleDate, 1, saleType)
